 ...
 * nubitsj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    /**
     * Returns the current context that is associated with the <b>calling thread</b>. nubitsj is an API that has thread
     ...
     * independently use nubitsj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using nubitsj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing nubitsj via a thread that has not had any context set on it.");
     ...
     * want to create core nubitsj objects. Generally, if a class can accept a Context in its constructor and might
     * be used (even indirectly) by a thread, you will want to call this first. Your task may be simplified by using
     * a {@link com.matthewmitchell.nubitsj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link com.matthewmitchell.core.NetworkParameters} specified when this context was (auto) created. The
     * network parameters defines various hard coded constants for a specific instance of a Nu network, such as
