...
import com.matthewmitchell.nubitsj.crypto.KeyCrypterScrypt;
...
import com.matthewmitchell.nubitsj.utils.ListenerRegistration;
...
import org.nubitsj.wallet.Protos.Wallet.EncryptionType;
...
import static com.matthewmitchell.nubitsj.core.Utils.nubitsValueToFriendlyString;
import static com.matthewmitchell.nubitsj.core.Utils.nubitsValueToPlainString;
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s NBT from our own wallet," +
                        " and sends us %s NBT", tx.getHashAsString(), Utils.nubitsValueToFriendlyString(valueSentFromMe),
                        Utils.nubitsValueToFriendlyString(valueSentToMe)));
            }
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} NBT: {} [{}] in block {}", sideChain ? " on a side chain" : "",
                nubitsValueToFriendlyString(valueDifference), tx.getHashAsString(), relativityOffset,
        ...
        if (!insideReorg && bestChain) {
            ...
            log.info("Balance is now: " + nubitsValueToFriendlyString(newBalance));
            if (!wasPending) {
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), new String(Hex.encode(tx.unsafeNubitsSerialize())));
                    Transaction other = input.getConnectedOutput().getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), new String(Hex.encode(tx.unsafeNubitsSerialize())));
                }
    ...
    public static class SendRequest {
         ...
         * is how mining is incentivized in later years of the Nubits system when inflation drops. It also provides
         ...
         *
         * <p>Once {@link Wallet#completeTx(com.matthewmitchell.nubitsj.core.Wallet.SendRequest)} is called, this is set to the
         ...
         * 
         * <p>Nubits: This is zero even though we add a required fee. This is an extra fee on top that we don't want.</p>
         */
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            if (bestChangeOutput != null) {
                ...
                log.info("  with {} coins change", nubitsValueToFriendlyString(bestChangeOutput.getValue()));
            }
            final BigInteger calculatedFee = totalInput.subtract(totalOutput);
            if (calculatedFee.compareTo(BigInteger.ZERO) > 0) {
                log.info("  with a fee of {}", nubitsValueToFriendlyString(calculatedFee));
            }
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s NBT (available: %s NBT) in:%n",
                    nubitsValueToPlainString(estimatedBalance), nubitsValueToPlainString(availableBalance)));
    ...
    private void toStringHelper(StringBuilder builder, Map<Sha256Hash, Transaction> transactionMap,
        ...
        for (Transaction tx : txns) {
            try {
                ...
                builder.append(Utils.nubitsValueToFriendlyString(tx.getValueSentFromMe(this)));
                builder.append(" and receives ");
                builder.append(Utils.nubitsValueToFriendlyString(tx.getValueSentToMe(this)));
                builder.append(", total value ");
                builder.append(Utils.nubitsValueToFriendlyString(tx.getValue(this)));
                builder.append(".\n");
            } catch (ScriptException e) {
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.nubitsValueToFriendlyString(balance));
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.matthewmitchell.nubitsj.crypto.KeyCrypter,
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            if (selection2 == null && selection1 == null) {
                ...
                log.warn("Insufficient value in wallet for send: needed {} more", nubitsValueToFriendlyString(valueMissing));
     ...
     * {@link com.matthewmitchell.nubitsj.core.PeerFilterProvider} interface and generally should not be used directly by apps.
