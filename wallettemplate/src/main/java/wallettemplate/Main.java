...
public class Main extends Application {
    ...
    public void setupWalletKit(@Nullable DeterministicSeed seed) {
        ...
        nubits = new WalletAppKit(params, new File("."), APP_NAME + "-" + params.getPaymentProtocolId()) {
            @Override
            protected void onSetupCompleted() {
                // Don't make the user wait for confirmations for now, as the intention is they're sending it
                // their own money!
                nubits.wallet().allowSpendingUnconfirmedTransactions();
                Platform.runLater(controller::onNuBitsSetup);
            }
        };
        ...
        if (seed != null)
            nubits.restoreWalletFromSeed(seed);
    }
