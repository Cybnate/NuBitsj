...
public class WalletSettingsController {
    ...
    public void restoreClicked(ActionEvent event) {
        ...
        if (Main.nubits.wallet().getBalance().value > 0) {
        ...
        informationalAlert("Wallet restore in progress",
                "Your wallet will now be resynced from the NuBits network. This can take a long time for old wallets.");
        overlayUI.done();
        ...
        // Shut down nubitsj and restart it with the new seed.
        Main.nubits.addListener(new Service.Listener() {
            @Override
            public void terminated(Service.State from) {
                Main.instance.setupWalletKit(seed);
                Main.nubits.startAsync();
            }
        }, Platform::runLater);
        Main.nubits.stopAsync();
    }
    ...
    public void passwordButtonClicked(ActionEvent event) {
        if (aesKey == null) {
            ...
            Main.nubits.wallet().decrypt(aesKey);
